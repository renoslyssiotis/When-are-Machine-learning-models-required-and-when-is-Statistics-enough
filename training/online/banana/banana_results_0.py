...
df1 = pd.read_csv('class0_banana.txt', 
                 sep="\t", header = None)
df2 = pd.read_csv('class1_banana.txt', 
                 sep="\t", header = None)
...
#import pickle
# with open(str(p.parents[2])+'/offline/banana/results_roc_banana_0.pickle', 'wb') as handle:
#     pickle.dump(results_roc, handle, protocol=pickle.HIGHEST_PROTOCOL)
# with open(str(p.parents[2])+'/offline/banana/results_prc_banana_0.pickle', 'wb') as handle:
#     pickle.dump(results_prc, handle, protocol=pickle.HIGHEST_PROTOCOL)
# with open(str(p.parents[2])+'/offline/banana/results_f1_banana_0.pickle', 'wb') as handle:
#     pickle.dump(results_f1, handle, protocol=pickle.HIGHEST_PROTOCOL)
# with open(str(p.parents[2])+'/offline/banana/metafeatures_banana_0.pickle', 'wb') as handle:
