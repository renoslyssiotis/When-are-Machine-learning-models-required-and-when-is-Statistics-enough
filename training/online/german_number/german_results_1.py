...
df1 = pd.read_csv('class0_german+numer.txt', 
                 sep="\t", header = None)
df2 = pd.read_csv('class1_german+numer.txt', 
                 sep="\t", header = None)
...
#import pickle
# with open(str(p.parents[2])+'/offline/german_number/results_roc_german_1.pickle', 'wb') as handle:
#     pickle.dump(results_roc, handle, protocol=pickle.HIGHEST_PROTOCOL)
# with open(str(p.parents[2])+'/offline/german_number/results_prc_german_1.pickle', 'wb') as handle:
#     pickle.dump(results_prc, handle, protocol=pickle.HIGHEST_PROTOCOL)
# with open(str(p.parents[2])+'/offline/german_number/results_f1_german_1.pickle', 'wb') as handle:
#     pickle.dump(results_f1, handle, protocol=pickle.HIGHEST_PROTOCOL)
# with open(str(p.parents[2])+'/offline/german_number/metafeatures_german_1.pickle', 'wb') as handle:
