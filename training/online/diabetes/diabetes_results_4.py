...
df1 = pd.read_csv('class0_diabetes.txt', 
                 sep="\t", header = None)
df2 = pd.read_csv('class1_diabetes.txt', 
                 sep="\t", header = None)
...
# with open(str(p.parents[2])+'/offline/diabetes/results_roc_diabetes_4.pickle', 'wb') as handle:
#     pickle.dump(results_roc, handle, protocol=pickle.HIGHEST_PROTOCOL)
# with open(str(p.parents[2])+'/offline/diabetes/results_prc_diabates_4.pickle', 'wb') as handle:
#     pickle.dump(results_prc, handle, protocol=pickle.HIGHEST_PROTOCOL)
# with open(str(p.parents[2])+'/offline/diabetes/results_f1_diabates_4.pickle', 'wb') as handle:
#     pickle.dump(results_f1, handle, protocol=pickle.HIGHEST_PROTOCOL)
# with open(str(p.parents[2])+'/offline/diabetes/metafeatures_diabates_4.pickle', 'wb') as handle:
