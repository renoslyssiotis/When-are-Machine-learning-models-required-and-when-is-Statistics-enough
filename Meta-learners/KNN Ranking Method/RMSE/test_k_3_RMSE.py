...
with open('/Users/renoslyssiotis/Desktop/When-are-ML-models-required-and-when-is-Statistics-enough-/Plots and Figures/Feature Selection/Univariate Selection/ANOVA_X_AUCPRC.pickle', 'rb') as handle:
...
for dataset in list(metadataset_feature_selected.index): #Dataset 0, Dataset 1, ..., Dataset 203                     
    
    summation = 0
    
    for index,value in enumerate(meta_features_list):  
        distance = (metadataset_feature_selected.loc[dataset][index] - meta_features_values[index])**2
        summation += distance
            
    distance_count.append(np.sqrt(summation/len(meta_features)))
...
print("mind_index_2: %d" %min_index_2)
min_index_3 = distance_count.index(sorted(distance_count)[2])    
print("mind_index_3: %d" %min_index_3)
